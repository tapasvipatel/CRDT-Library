...
#define NUM_PRIORITY_LIST_BOARDS 3
...
void deletePnCountersTable();
void deleteMultisetsTable();
void createPnCounterTable();
void createMultisetsTable();
void insertMultisetsTable(string boardType, string element, string priority);
void updateTableMaster(tgui::GuiBase &gui);
void updatePnCountersTable(string boardType, string newValue);
...
string getTask(string element) {
    
    vector<char> task;
    bool parse = false;
    for (int i = 0; i < element.length(); i++) {
        if (parse) {
            task.push_back(element[i]);
        }
        if (element[i] == '.') {
            parse = true;
        }
    ...
    string taskString(task.begin(), task.end());
    return taskString;
}
...
void updateTableMaster(tgui::GuiBase &gui)
{
    ...
    for(auto element : backlogPayload)
    {
        ...
        insertMultisetsTable("backlog", getTask(element), to_string(getPriority(element)));
    ...
    for(auto element : inprogressPayload)
    {
        ...
        insertMultisetsTable("inprogress", getTask(element), to_string(getPriority(element)));
    ...
    for(auto element : readytotestPayload)
    {
        ...
        insertMultisetsTable("readytotest", getTask(element), to_string(getPriority(element)));
    ...
    for(auto element : completePayload)
    {
        ...
        insertMultisetsTable("complete", getTask(element), to_string(getPriority(element)));
    ...
    for(auto element : notaddedPayload)
    {
        ...
        insertMultisetsTable("notadded", getTask(element), to_string(getPriority(element)));
    ...
    {
        if (iteration == NUM_PRIORITY_LIST_BOARDS)
        {
            break;
        }
    ...
    updatePnCountersTable("backlog", to_string(numTasksBacklogServer.queryPayload()));
    ...
    updatePnCountersTable("inprogress", to_string(numTasksInprogressServer.queryPayload()));
    ...
    updatePnCountersTable("readytotest", to_string(numTasksReadytotestServer.queryPayload()));
    ...
    updatePnCountersTable("complete", to_string(numTasksCompleteServer.queryPayload()));
    ...
    updatePnCountersTable("notadded", to_string(numTasksNotaddedServer.queryPayload()));
}
...
void deletePnCountersTable()
{
    string pnCountersTable = "DROP TABLE " + endUser.getUserName() + "_pncounters";
    PGresult *res = PQexec(conn, pnCountersTable.c_str());

    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        cout << "Could not delete pncounters table!" << endl;
        PQfinish(conn);
        exit(1); 
    }
    else {
        cout << "Deleted pncounters table!" << endl;
    }
...
void deleteMultisetsTable()
{
    string multisetTables = "DROP TABLE " + endUser.getUserName() + "_multisets";
    PGresult *res = PQexec(conn, multisetTables.c_str());

    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        cout << "Could not delete multisets table!" << endl;
        PQfinish(conn);
        exit(1); 
    }
    else {
        cout << "Deleted multisets table!" << endl;
    }
...
void updatePnCountersTable(string boardType, string newValue) {
    ...
    string tableName = endUser.getUserName() + "_pncounters";
    string command = "UPDATE " + tableName + " SET numboards = " + newValue + " WHERE boardtype = '" + boardType + "'";

    PGresult *res = PQexec(conn, command.c_str());

    if (PQresultStatus(res) != PGRES_COMMAND_OK) {

        cout << "Could not update pncounter table with value " << newValue << endl;
        cout << PQerrorMessage(conn) << endl;
        PQclear(res);
        PQfinish(conn);
        exit(1);
        ...
        cout << "Updated pncounter table with value " << newValue << endl;
    }
...
void insertMultisetsTable(string boardType, string element, string priority)
{
    ...
    string tableName = endUser.getUserName() + "_multisets";
    string command = "INSERT INTO " + tableName + " (boardType, task, priority) VALUES('" + boardType + "','" + element + "'," + priority + ")";
    PGresult *res = PQexec(conn, command.c_str());
        
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        cout << "Could not insert " << element << " into multisets table!" << endl;
        cout << PQerrorMessage(conn) << endl;
        PQfinish(conn);
        exit(1);     
    }
    else {
        cout << "Inserted " << element << " into multisets table." << endl;
    }

    PQclear(res);
}

void insertPnCountersTable(string tableName, string boardType, string element)
{

    string command = "INSERT INTO " + tableName + " (boardtype, numboards) VALUES('" + boardType + "'," + element + ")";
    PGresult *res = PQexec(conn, command.c_str());
        
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        cout << "Could not insert " << element << " into pncounters table!" << endl;
        PQfinish(conn);
        exit(1);     
    }
    else {
        cout << "Inserted " << element << " into pncounters table." << endl;
    }

    PQclear(res);
}

void insertElementIntoTable(string tableName, string boardType, string element)
{
    if (tableName == (endUser.getUserName() + "_pncounters"))
    {
        insertPnCountersTable(tableName, boardType, element);
    }
}

void createPnCounterTable()
{
    string pnCountersTable = "CREATE TABLE " + endUser.getUserName() + "_pncounters(boardtype CHAR(15), numboards INT)";
    PGresult *res = PQexec(conn, pnCountersTable.c_str());

    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        cout << "Could not create pncounters table!" << endl;
        PQfinish(conn);
        exit(1); 
    }
    else {
        cout << "Created pncounters table!" << endl;
        string tableName = endUser.getUserName() + "_pncounters";
        insertElementIntoTable(tableName, "backlog", "0");
        insertElementIntoTable(tableName, "inprogress", "0");
        insertElementIntoTable(tableName, "readytotest", "0");
        insertElementIntoTable(tableName, "complete", "0");
        insertElementIntoTable(tableName, "notadded", "0");
    }
...
void createMultisetsTable()
{
    string multisetTables = "CREATE TABLE " + endUser.getUserName() + "_multisets(boardtype CHAR(15), task CHAR(50), priority INT)";
    PGresult *res = PQexec(conn, multisetTables.c_str());

    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        cout << "Could not create multisets table!" << endl;
        PQfinish(conn);
        exit(1); 
    }
    else {
        cout << "Created multisets table!" << endl;
    }
...
void login(tgui::EditBox::Ptr username, tgui::EditBox::Ptr password, tgui::GuiBase &gui, tgui::Label::Ptr &message)
{
    ...
    if (createUser.doesExist((string)username->getText(), (string)password->getText()))
    {
        ...
        string connectionString = "user=vishcapstone dbname=crdtdb";
        ...
        if (PQstatus(conn) == CONNECTION_BAD) {
            ...
            cout << "Connected to database successfully!" << std::endl;
            createPnCounterTable();
            createMultisetsTable();
        }
